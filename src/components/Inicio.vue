          ...
          <v-container>
            <v-row>
              ...
              <v-col class="text-right" cols="0" sm>
                <div class="my-2">
                  <v-btn dark @click="cadastrar">Nova</v-btn>
                </div>
              </v-col>
            </v-row>
          </v-container>
...
export default {
  ...
  methods: {
    carregar(){
      axios({
      method: "get",
      url: `${constants.baseUrl}/atendimento`
    })
      .then(response => {
    ...
    deletar: function(value) {
      let id = value.id;
      if(confirm(`Deseja excluir o chamado nº ${id}?`) == true){
      axios
        .delete(`${constants.baseUrl}/atendimento/destroy/${id}`)
        .then(response => {
    ...
    consultar:function(item){
      const id = item.id
      this.$router.push({ path:`/consultar/${id}`})
    },
    ...
    finalizar:function(item){
      const id = item.id
      this.$router.push({ path:`/finalizar/${id}`})
    }
  }
};
</script>
